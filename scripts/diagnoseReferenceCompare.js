...
(async () => {
  try {
    ...
    const { decrypt } = require(path.resolve(__dirname, '../backend/src/utils/encryption'));
    ...
    if (!templateId || !tenantArg) {
      console.log('Usage: node scripts/diagnoseReferenceCompare.js <template-id> <tenant-db-id-or-tenant-id>');
      process.exit(1);
    }
    ...
    let tenant = db.prepare('SELECT * FROM tenants WHERE id = ?').get(tenantArg);
    if (!tenant) tenant = db.prepare('SELECT * FROM tenants WHERE tenant_id = ?').get(tenantArg);
    if (!tenant) {
      console.error('Tenant not found in DB:', tenantArg);
      process.exit(3);
    }
    ...
    try {
      ...
      try {
        ...
        if (err instanceof LicenceUnavailableError || err.code === 'LICENCE_UNAVAILABLE') {
          console.log(JSON.stringify({ templateId: tpl.id, tenantId: tenant.id, note: err.message }, null, 2));
          process.exit(0);
        }
        throw err;
      }

      // Reuse compare helpers from the route logic
      ...
      if (!Array.isArray(watched) || watched.length === 0) {
        console.log('Template has no watched_keys to scan.');
      }
      ...
      for (const refKey of Object.keys(refResources)) {
        ...
        if (!Array.isArray(watched) || watched.length === 0) {
          items.push({ refId: refKey, refDisplayName: ref.displayName || '', status: 'no_watched_keys', note: 'No watched_keys to scan' });
          continue;
        }

        // Only consider watched keys that are relevant to this specific reference entry.
        const relevant = Array.isArray(watched) ? watched.filter(wk => getByPath(ref, wk.path) !== undefined) : [];
        if (relevant.length === 0) {
          items.push({ refId: refKey, refDisplayName: ref.displayName || '', status: 'no_watched_keys', note: 'No watched_keys present in reference' });
          continue;
        }
      ...
      console.log(JSON.stringify({ templateId: tpl.id, tenantId: tenant.id, summary: { total, matched: matchedCount, mismatched: mismatchedCount, missing: missingCount }, items, sampleLiveResourceCount: Object.keys(liveMap).length }, null, 2));
      process.exit(0);
    } catch (err) {
      console.error('Reference compare diagnostic failed:', err && err.message ? err.message : String(err));
      process.exit(5);
    }
  } catch (err) {
