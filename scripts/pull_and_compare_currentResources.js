...
async function run(tplId, tenantId) {
  ...
  console.log('Posting compare with currentResources...');
  const cmp = await req('POST', `/api/reference-templates/${encodeURIComponent(tplId)}/compare`, { currentResources: live.resources, scan: true });
...
if (process.argv.length < 4) { console.error('Usage: node scripts/pull_and_compare_currentResources.js <templateId> <tenantInternalId>'); process.exit(2); }
