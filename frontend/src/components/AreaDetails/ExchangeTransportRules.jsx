...
export default function ExchangeTransportRulesArea({ tenantId, areaKey, liveData, baseline }) {
  if (!liveData) {
    return (
      <div>
        <h3 className="text-lg font-semibold">Transport Rules</h3>
        <p className="text-sm text-gray-400">Area key: {areaKey}</p>
        <p className="mt-2 text-xs text-gray-500">No live snapshot yet. Use the <strong>Pull Live Data</strong> button to attempt fetching transport rules from Microsoft Graph (beta) where available.</p>
      </div>
    )
  }

  // We intentionally do not render numeric summary cards for transport rules.
  const resources = liveData.resources || {}
  const ids = Object.keys(resources)
  const total = ids.length
  const enabledCount = ids.reduce((s, id) => s + (resources[id]?.isEnabled ? 1 : 0), 0)
    ...
    <div>
      {total === 0 ? null : (
        <div className="card p-3">
          <div className="text-sm font-medium text-gray-300">Transport rules</div>
              ...
              return (
                <li key={id} onClick={() => openDetail(id)} className="flex items-start justify-between cursor-pointer">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-gray-500">{r.priority !== null && r.priority !== undefined ? `Priority: ${r.priority}` : ''}</div>
                  </div>
                  <div className="text-xs text-gray-400 ml-3">{r.isEnabled === null || r.isEnabled === undefined ? 'unknown' : (r.isEnabled ? 'enabled' : 'disabled')}</div>
                </li>
              )
            })}
          </ul>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-11/12 max-w-3xl bg-gray-900 border border-gray-800 rounded-lg p-4 text-sm">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-white">{detail?.displayName || 'Transport rule'}</h3>
