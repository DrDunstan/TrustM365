...
(async function main() {
  try {
    ...
    // Force export and write to disk immediately to avoid async save bug
    try {
      const buf = db._db.export();
      fs.writeFileSync(DB_PATH, Buffer.from(buf));
      console.log('[persist] Database exported to disk');
    } catch (err) {
      console.error('[persist] direct export failed', err.message);
    }
